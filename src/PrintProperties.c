...
ULONG
WINAPI
PrintSessionSecurityInfo(TRACEHANDLE TraceHandle)
{
    BOOL bRes = 0;
    NTSTATUS Status;
    ...
    {
        if (SessionInfo)
            RtlFreeHeap(HeapHandle, 0, SessionInfo);
        SessionInfo = RtlAllocateHeap(HeapHandle, HEAP_ZERO_MEMORY, InfoLen);
